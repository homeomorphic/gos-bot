...
public class Round {
    ...
    public Player play() throws IOException {
        final Process[] processes = new Process[2];
        ...
        outs = new OutputStreamWriter[2];
        for (int i = 0; i < processes.length; i++) {
            ins[i] = new BufferedReader(new InputStreamReader(processes[i].getInputStream()));
            outs[i] = new OutputStreamWriter(processes[i].getOutputStream());
        }
        ...
        outs[0].write(gson.toJson(whiteRequest));
        outs[1].write(gson.toJson(blackRequest));
    ...
    private void allowPlayBy(Player player, MoveType[] types) throws IOException {
        ...
        final OutputStreamWriter out = outs[player == Player.White ? 0 : 1];
        final MoveRequest moveRequest = new MoveRequest(board, types);
        out.write(gson.toJson(moveRequest));
        ...
        outs[0].write(processedMoveStr);
        outs[0].flush();
        outs[1].write(processedMoveStr);
        outs[1].flush();
    }
    ...
    private void applyMove(Player player, Move move) {
        switch (move.Type) {
            case Attack:
                //board.SetSpace(location, player, stone, height);
